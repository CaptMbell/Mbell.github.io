...
switch($_GET[act]){
			...
			<table id="tablekonten" class="table table-striped table-bordered table-responsive" style="">
				...
				<tbody>
					<?php 
					    ...
					    $tampil = mysqli_query($conn, "SELECT * FROM tgroup order by groupName asc limit $posisi,$batas");
					    $no =$posisi+1;
						...
						$jmldata = mysqli_num_rows(mysqli_query($conn ,"SELECT * FROM tgroup"));
  ...
  // Form Edit group
  case "editgroup":
    $edit=mysqli_query($conn , "SELECT * FROM tgroup WHERE groupId='$_GET[id]'");
