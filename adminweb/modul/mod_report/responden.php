...
$hasil = mysqli_query($conn , "SELECT * FROM tdescription");
$date = date('Y-m-d');
$time = date('H:i:s');


$responden = mysqli_fetch_array(mysqli_query($conn , "SELECT * FROM tanswer, tcompany WHERE tanswer.companyId = '$_GET[id]' AND tcompany.companyId = tanswer.companyId"));
...
echo "<center><table border=0 cellpadding=10 cellspacing=3 bgcolor= #e6e6e6>
	...
	while ($data = mysqli_fetch_array($hasil)){
      ...
      $e=rubah($oke[TotalE]);
			...
			$data_count = mysqli_fetch_array(mysqli_query($conn , "SELECT 
...
{
	include "../../../koneksi.php";
	mysqli_query($conn ,"DELETE FROM tcompany WHERE companyId='$_GET[id]'");
