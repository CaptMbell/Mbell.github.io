...
<div class="row">
    <div class="col-lg-12">
        ...
        <ol class="breadcrumb">
        	...
        	<?php if ($_GET['sub']=='all'){ ?>
            <li class="active">
                 <a href="master.php?module=hasil&sub=all">Keseluruhan</a>
            </li>
            <?php } ?>
			<?php if ($_GET['sub']=='pergroup'){ ?>
            <li class="active">
                 <a href="master.php?module=hasil&sub=pergroup">Per Group</a>
            </li>
            <?php } ?>
...
<nav class="navbar navbar-inverse" >
	<ul class="nav navbar-nav">
		<li class="<?php if($_GET['sub']=='all'){echo'active';} ?>"><a href="?module=hasil&sub=all">Keseluruhan</a></li>
		<li class="<?php if($_GET['sub']=='pergroup'){echo'active';} ?>"><a href="?module=hasil&sub=pergroup">Per Group</a></li>
...
<?php if ($_GET['sub']=='all'){ ?>
	<div class="panel panel-primary">
		<div class="panel-heading">
			<div class="panel-title">Kuisioner Secara Keseluruhan</div>
		</div>
		...
		<div class="panel-body">
			...
			$sql = mysqli_query($conn, "SELECT SUM(jawabanA) As TotalA,
			...
			$des = mysqli_num_rows(mysqli_query($conn, "SELECT * FROM tdescription"));
			$noo=1;
...
<?php if ($_GET['sub']=='pergroup'){ ?>
		...
		<?php
		error_reporting(1);
		$result=mysqli_query($conn, "SELECT groupId from tgroup group by groupId ");
		$kolom = 2;
		$array=array();
		...
		$chunks=array_chunk($array, $kolom);
			...
			foreach ($chunks as $chunk) {
			    foreach ($chunk as $data) {
			        ...
			        $data2=mysqli_fetch_array(mysqli_query($conn , "SELECT *from tgroup where groupId='$data[groupId]' group by groupId "));
			        ?>
					<div class="panel panel-primary" style='margin-left:10px'>
						...
						<div class="panel-body">
							...
							$nom = mysqli_num_rows(mysqli_query($conn, "SELECT * FROM tanswer where groupId='$data2[groupId]'"));
						...
						?>
							...
							<table id="tablekonten" class="table table-striped table-bordered">
						         ...
						         <th><div id='kontentd'>Aksi</div></th></tr>
									...
									error_reporting(1);
										$jumlahdata = mysqli_num_rows(mysqli_query($conn , "SELECT * FROM tcompany WHERE dateSurvey BETWEEN '$tgl_awal' AND '$tgl_akhir'"));
										$sql = mysqli_query($conn, "SELECT * FROM tcompany WHERE dateSurvey BETWEEN '$tgl_awal' AND '$tgl_akhir' ORDER by companyName ");
										$no =1;
						         ...
						         <th><div id='kontentd'>Aksi</div></th></tr>
									<?php
											...
											error_reporting(1);
												...
												$jumlahdata = mysqli_num_rows(mysqli_query($conn , "SELECT * FROM tcompany "));
												$p      = new PagingHasil;
												$batas  = 10;
												$posisi = $p->cariPosisi($batas);
												$sql = mysqli_query($conn , "SELECT * FROM tcompany  ORDER by companyName ASC LIMIT $posisi,$batas");
												$no = $posisi+1;
											while ($data = mysqli_fetch_array($sql)){
												$dateIndo = tgl_indo($data['dateSurvey']);
												?>
							...
							<?php
								...
								$jmldata = mysqli_num_rows(mysqli_query($conn , "SELECT * FROM tcompany "));
