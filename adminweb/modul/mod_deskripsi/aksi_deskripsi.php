...
if ($module=='description' AND $act=='hapus'){
	mysqli_query($conn,"DELETE FROM tdescription WHERE descriptionId='$_GET[id]'");
...
elseif ($module=='description' AND $act=='input'){
	...
	elseif (empty($deskripsi)){
		...
		$masuk = mysqli_query($conn,"INSERT INTO tdescription(groupId,description,CreatedDate,CreatedUser) VALUES('$groupId','$deskripsi','$createdDate','$_SESSION[userId]')");
		if($masuk){
			header('location:../../master.php?module=description');
		}
		else{
...
elseif ($module=='description' AND $act=='update'){
	...
	$aksi=mysqli_query($conn,"UPDATE tdescription SET description='$description', groupId='$groupId',ModifiedDate = '$modifiedDate',
