...
switch($_GET[act]){
	...
	<div class="panel panel-primary">
		<div class="panel-heading">
			...
			<div class="panel-title"><span class="glyphicon glyphicon-list"></span> Manajemen Deskripsi <i style="margin-left:750px;"><?php if($_SESSION['level']=="Super"){?><button class="btn btn-success btn-sm " onclick="window.location.href='?module=description&act=tambahpertanyaan'"><span class="glyphicon glyphicon-plus"></span> Tambah Deskripsi </button></i><?php }?></div>
		</div>
			...
			<table id="tablekonten" class="table table-striped table-bordered table-responsive" style="">
				...
				<tbody>
					<?php 
					    ...
					    $tampil = mysqli_query($conn, "SELECT * FROM tdescription order by groupId asc limit $posisi,$batas ");
					    $no = $posisi+1;
						...
						$jmldata = mysqli_num_rows(mysqli_query($conn, "SELECT * FROM tdescription"));
					...
					<div class="col-sm-5">
						<div class="input-group">
							...
							<select name="groupId" id="" class="form-control">
								<?php 
								$sql = mysqli_query($conn , "SELECT * FROM tgroup ORDER BY groupId");
  ...
  case "editdescription":
    $edit = mysqli_query($conn , "SELECT * FROM tdescription WHERE descriptionId='$_GET[id]'");
