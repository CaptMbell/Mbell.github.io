...

{
  ...
  $passbaru=md5($_POST['newpass']);
    $aksi =mysqli_query($conn , "UPDATE tuser SET password= '$passbaru' WHERE userId='$user'");
  if($aksi==true)
  {
...
else
{
  ...
  $passdb =mysqli_fetch_array(mysqli_query($conn , "SELECT password from tuser where userId='$_SESSION[userId]'"));
